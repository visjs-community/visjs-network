...
class ClusterEngine {
  ...
  /**
   * Using a cluster edgeId, update the clustered edge with the new options
   * @param {vis.Edge.id} clusterEdgeId
   * @param {object} newOptions
   */
  updateClusterEdge(clusterEdgeId, newOptions) {
    if (clusterEdgeId === undefined) {
      throw new Error('No clusterEdgeId supplied to updateClusterEdge.')
    }
    if (newOptions === undefined) {
      throw new Error('No newOptions supplied to updateClusterEdge.')
    }
    if (this.body.edges[clusterEdgeId] === undefined) {
      throw new Error('The clusterEdgeId supplied to updateClusterEdge does not exist.')
    }

    var edge = this.body.edges[clusterEdgeId];
    util.extend(edge.options, newOptions);
    this.body.emitter.emit('_dataChanged')
  }
